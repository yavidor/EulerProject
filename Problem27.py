...
expression that produces the maximum number of primes for consecutive values of , starting with n=0.
"""
...
for i in range(-1000, 1000):
    for j in range(-1001, 1001):
        if prime(j):
            temp = countprimes(i, j)
            if temp > maxprime:
                maxprime, maxprod = temp, i * j
print(maxprod)
