...
grid = '08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 ' \
       ...
       '89 19 67 48'
grid = np.array(grid.split(' '))
grid.shape = (20, 20)
largest = 0
grid = grid.astype(np.int)
diagonally = 0
for y in range(16, 2, -1):
    for x in range(16, 2, -1):
        diagonally = max(diagonally, grid[y][x] * grid[y - 1][x + 1] * grid[y - 2][x + 2] * grid[y - 3][x + 3])
for y in range(16):
    for x in range(16):
        horizontal = grid[y][x] * grid[y][x + 1] * grid[y][x + 2] * grid[y][x + 3]
        vertical = grid[y][x] * grid[y + 1][x] * grid[y + 2][x] * grid[y + 3][x]
        diagonal = grid[y][x] * grid[y + 1][x + 1] * grid[y + 2][x + 2] * grid[y + 3][x + 3]
        largest = max(largest, horizontal, vertical, diagonal, diagonally)
print(largest)
